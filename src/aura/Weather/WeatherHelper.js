({
    ...
    getWeatherOfSelectedContact : function(component, event, helper){
        var action = component.get("c.getWeather");
        var contact = event.getParam("contact");
        if(contact.MailingLatitude != null && contact.MailingLongitude != null){
            action.setParams({
                "q" : contact.MailingLatitude+','+contact.MailingLongitude
            });
